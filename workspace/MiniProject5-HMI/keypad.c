	...
	while(1)
	{
		for(col=0;col<KEYPAD_NUM_COLS;col++) /* loop for columns */
		{
			...
			for(row=0;row<KEYPAD_NUM_ROWS;row++) /* loop for rows */
			{
				...
				{
					#if (KEYPAD_NUM_COLS == 3)
						uint8 a_key=KEYPAD_4x3_adjustKeyNumber((row*KEYPAD_NUM_COLS)+col+1);
					#elif (KEYPAD_NUM_COLS == 4)
						uint8 a_key=KEYPAD_4x4_adjustKeyNumber((row*KEYPAD_NUM_COLS)+col+1);
					#endif
...
/*
 * Description :
 * Update the keypad pressed button value with the c8/orrect one in keypad 4x4 shape
 */
static uint8 KEYPAD_4x4_adjustKeyNumber(uint8 button_number)
{
	uint8 keypad_button = 0;
